"""
SEMA-CL Jittor version - Trainer.
"""
import sys
import logging
import copy
...
def _set_device(args):
    """Jittor uses flags to select device instead of explicit device objects."""
    device_type = args["device"]
    gpus = []
    use_cuda = False
    for device in device_type:
        if device == -1 or str(device) == "-1" or str(device) == "cpu":
            gpus.append("cpu")
        else:
            use_cuda = True
    ...
    if use_cuda:
        jt.flags.use_cuda = 1
        # If specific GPU is needed: os.environ["CUDA_VISIBLE_DEVICES"] = gpus[0]
    else:
        jt.flags.use_cuda = 0
